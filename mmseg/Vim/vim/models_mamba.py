...
# from rope import *
from mmseg.Vim.vim.rope import *
import random
...
if __name__ == '__main__':
    ...
    x = torch.rand(2, 3, 1024, 1024).to(device)
    B, C, H, W = x.shape
    print("输入特征维度：", x.shape)    # torch.Size([2, 3, 1024, 1024])
    x = x.view(B, C, H * W).permute(0, 2, 1)    
    print("维度变换：", x.shape)    # torch.Size([2, 1048576, 3])
    ...
    print("输出特征维度：", x.shape)    # 输出特征维度： torch.Size([2, 3, 1024, 1024])