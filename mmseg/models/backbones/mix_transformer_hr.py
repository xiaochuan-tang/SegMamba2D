...
class MixVisionTransformer(nn.Module):
    ...
    def forward_seg_hr_mamba_v4(self, x):
        # print("x.shape:",x.shape)   # torch.Size([2, 3, 469, 469])
        ...
        # stage 1
        x_1, H_1, W_1 = self.patch_embed1(x)
        # print("x_1.shape:",x_1.shape)   # torch.Size([2, 4096, 64])
        for i, blk in enumerate(self.block1):
            x_1 = blk(x_1, H_1, W_1)
        x_1 = self.norm1(x_1)
        # print("x_1.shape:",x_1.shape)   # torch.Size([2, 4096, 64])
        ...
        for i, blk in enumerate(self.block1_1):
            x_1_1 = blk(x_1, H_1, W_1)
        x_1_1 = self.norm1_1(x_1_1)
        # print("x_1_1.shape:",x_1_1.shape)   # torch.Size([2, 4096, 64])
        x_1_1_reshape = x_1_1.reshape(B, H_1, W_1, -1).permute(0, 3, 1, 2).contiguous()
        x_1_1_reshape = F.relu(x_1_1_reshape)  # 添加 ReLU 操作
        # print("x_1_1_reshape.shape:",x_1_1_reshape.shape)   # torch.Size([2, 64, 64, 64])

        # stage 2
        x_2, H_2, W_2 = self.patch_embed2(x_1_reshape)
        # print("x_2.shape:",x_2.shape)   # torch.Size([2, 1024, 128])
        for i, blk in enumerate(self.block2):
            x_2 = blk(x_2, H_2, W_2)
        x_2 = self.norm2(x_2)
        # print("x_2.shape:",x_2.shape)   # torch.Size([2, 1024, 128])
        ...
        # print("x.shape:",x.shape)   # torch.Size([2, 3, 256, 256])
        # outs.append(x_2_reshape)
        # print("x_2_reshape.shape:",x_2_reshape.shape)   # torch.Size([2, 128, 64, 64])
    ...
    def forward(self, x):
        ...
        x = self.forward_seg_hr_mamba_v4(x)    # 在forward_seg_hr_mamba的基础上,调整特征融合操作、添加relu（直接将relu操作后的特征图放入解码器）、
        # x = self.head(x)
