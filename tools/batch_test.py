...
def parse_args():
    ...
    parser.add_argument('--config', default="local_configs/segformer/B3/GDCLD.py", help='test config file path')
    parser.add_argument('--checkpoint-dir', default="output/GDCLD/segmamba/", help='directory containing checkpoint files')
...
def test_checkpoint(cfg, checkpoint_path, args):
    ...
    efficient_test = args.eval_options.get('efficient_test', False) if args.eval_options else False

    if not args.launcher == 'none':
    ...
    if rank == 0:
        ...
        if args.format_only:
            dataset.format_results(outputs, **(args.eval_options or {}))
        if args.eval:
            dataset.evaluate(outputs, args.eval, **(args.eval_options or {}))
...
def main():
    ...
    if args.aug_test:
        # Augmentation configuration as needed (same as your original code)
        pass
    ...
    for checkpoint_path in checkpoint_files:
        ...
        test_checkpoint(cfg, checkpoint_path, args)
